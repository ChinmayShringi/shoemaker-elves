...
from gpt_agent_orchestrator.planners.registry import (
    create_planner,
    get_available_providers,
    register_planner,
)
from gpt_agent_orchestrator.planners.anthropic_adapter import AnthropicAdapter
from gpt_agent_orchestrator.planners.azure_openai_adapter import AzureOpenAIAdapter
...
def test_create_openai_planner():
    ...
    with patch("src.gpt_agent_orchestrator.planners.openai_adapter.OpenAI"):
...
def test_create_anthropic_planner():
    ...
    with patch("src.gpt_agent_orchestrator.planners.anthropic_adapter.Anthropic"):
        planner = create_planner(
            provider="anthropic",
            api_key="test-key",
...
def test_create_azure_openai_planner():
    ...
    with patch("src.gpt_agent_orchestrator.planners.azure_openai_adapter.AzureOpenAI"):
        planner = create_planner(
...
def test_load_plugins_with_mock_entrypoint():
    ...
    from gpt_agent_orchestrator.planners.registry import load_plugins, _REGISTRY
    from gpt_agent_orchestrator.planners.types import ReviewSpec, TaskSpec
    ...
    with patch("src.gpt_agent_orchestrator.planners.registry.entry_points", return_value=mock_eps):
...
def test_plugin_cannot_override_builtin():
    ...
    from unittest.mock import Mock, patch
    from gpt_agent_orchestrator.planners.registry import load_plugins
    ...
    with patch("src.gpt_agent_orchestrator.planners.registry.entry_points", return_value=mock_eps):
        with patch("src.gpt_agent_orchestrator.planners.registry.logger") as mock_logger:
            load_plugins()
...
def test_entrypoint_discovery_failure():
    ...
    with patch("src.gpt_agent_orchestrator.planners.registry.entry_points", side_effect=RuntimeError("Discovery failed")):
        with patch("src.gpt_agent_orchestrator.planners.registry.logger") as mock_logger:
            load_plugins()
            # Should have logged a warning
            mock_logger.warning.assert_called()
...
def test_plugin_with_python39_entrypoints_api():
    ...
    with patch("src.gpt_agent_orchestrator.planners.registry.entry_points", return_value=mock_eps_dict):
        load_plugins()
