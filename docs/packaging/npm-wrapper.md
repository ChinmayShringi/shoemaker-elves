...
- `packages/npm/bin/gpt-orch.js` (new) - wrapper script
...
### Assumptions
- Binaries are published to GitHub Releases with naming convention: `gpt-orch-{platform}-{arch}` (e.g., `gpt-orch-darwin-arm64`, `gpt-orch-linux-x64`, `gpt-orch-win-x64.exe`)
...
### Files Modified
- `packages/npm/.gitignore` — Updated to use `bin/gpt-orch-*` pattern instead of specific binary names
...
- `packages/npm/bin/gpt-orch.js` — Wrapper script that spawns platform-specific binary with proper signal handling
...
#### Wrapper Script (`bin/gpt-orch.js`)
...
- **Issue**: Original `.gitignore` listed specific binary names
  - **Resolution**: Updated to use wildcard pattern `bin/gpt-orch-*` to cover all platform binaries
