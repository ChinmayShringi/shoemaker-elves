...
2. Add config defaults:
   ...
   - Map `GPT_ORCH_DEEPSEEK_API_KEY` environment variable to api_key
...
- `src/gpt_agent_orchestrator/planners/registry.py` - Add deepseek provider mapping
- `src/gpt_agent_orchestrator/config.py` - Add DeepSeek defaults and env var mapping
...
3. Added `GPT_ORCH_DEEPSEEK_API_KEY` environment variable support in config system
...
- `src/gpt_agent_orchestrator/planners/registry.py` — Added `_deepseek_factory()` and registered "deepseek" provider
- `src/gpt_agent_orchestrator/config.py` — Added "deepseek" to `PROVIDER_API_KEY_ENV_VARS` and implemented `_apply_provider_defaults()` function
- `src/gpt_agent_orchestrator/cli.py` — Updated provider help text and added DeepSeek base_url prompt in init command
...
✅ `--planner-provider deepseek` uses the DeepSeek API key from `GPT_ORCH_DEEPSEEK_API_KEY`
