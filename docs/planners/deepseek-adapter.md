...
2. Add config defaults:
   ...
   - Map `SHOEMAKER_ELVES_DEEPSEEK_API_KEY` environment variable to api_key
...
- `src/shoemaker-elves/planners/registry.py` - Add deepseek provider mapping
- `src/shoemaker-elves/config.py` - Add DeepSeek defaults and env var mapping
...
3. Added `SHOEMAKER_ELVES_DEEPSEEK_API_KEY` environment variable support in config system
...
- `src/shoemaker-elves/planners/registry.py` — Added `_deepseek_factory()` and registered "deepseek" provider
- `src/shoemaker-elves/config.py` — Added "deepseek" to `PROVIDER_API_KEY_ENV_VARS` and implemented `_apply_provider_defaults()` function
- `src/shoemaker-elves/cli.py` — Updated provider help text and added DeepSeek base_url prompt in init command
...
✅ `--planner-provider deepseek` uses the DeepSeek API key from `SHOEMAKER_ELVES_DEEPSEEK_API_KEY`
